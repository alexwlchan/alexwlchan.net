---
layout: article
date: 2025-01-17 10:54:23 +00:00
...
topic: Rust
---
I've posted another command-line tool: [randline], which gives you a random selection of lines in a file:
...
```python {"names":{"1":"random","2":"sys","3":"lines","8":"k","13":"k"}}
import random
import sys
...
<div>
$$
\begin{equation*}\text{probability of selecting this }k\text{-subset}=\frac{\text{# of permutations where this subset is the first }k\text{ items}}{\text{# of permutations}}\end{equation*}
$$
</div>
{% endraw %}
...
It's a bit too much code to include here, but you can read [my Rust implementation][my_code].
...
[my_code]: /projects/randline/files/src/sampling.rs
...
```rust {"names":{"1":"reservoir_sample","2":"T","3":"items","8":"k"}}
...
```rust {"names":{"1":"reservoir_sample","2":"T","3":"items","8":"k","11":"letters","12":"items"}}
fn reservoir_sample<T>(
    mut items: impl Iterator<Item = T>,
    k: usize) -> Vec<T> { … }
...
```rust {"names":{"1":"WeightedItem","2":"T","3":"item","5":"weight","6":"reservoir"}}
...
But in case you're interested, I've [posted all the code][github].
