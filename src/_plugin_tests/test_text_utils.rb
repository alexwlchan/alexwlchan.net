...
require_relative '../_plugins/utils/text'

class TestTextUtils < Test::Unit::TestCase
  def test_it_adds_non_breaking_spaces_after_words
    text = 'Apollo 11 launched in 1969'
    expected = 'Apollo&nbsp;11 launched in 1969'
    assert_equal(Alexwlchan::TextUtils.add_non_breaking_characters(text), expected)
  end
  ...
  def test_preserves_language_console
    ...
    assert_equal(Alexwlchan::TextUtils.cleanup_syntax_highlighter_classes(text), expected)
  end
...
HTML
    actual = Alexwlchan::TextUtils.cleanup_syntax_highlighter_classes(text)
